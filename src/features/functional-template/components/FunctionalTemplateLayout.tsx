...
export function FunctionalTemplateLayout({ templateTitle, templateId, template, practice }: FunctionalTemplateLayoutProps) {
  ...
  return (
    <div className={`flex h-screen flex-col transition-colors duration-300 ${isDarkMode ? 'bg-[#0F172A]' : 'bg-white'}`}>
          ...
          hasPracticeFiles ? (
            <div className="relative flex flex-1 min-w-0 overflow-hidden">
              ...
              <div
                ...
                className={`w-1 cursor-col-resize bg-gray-200 transition-colors duration-150 hover:bg-purple-500 ${isDarkMode ? 'bg-gray-700 hover:bg-purple-400' : ''}`}
              />
      ...
      <div className={`flex h-16 items-center border-t px-6 transition-colors duration-300 ${isDarkMode ? 'bg-[#0F172A] border-[#334155]' : 'bg-white border-[#F1F5F9]'}`}>
        <button
          onClick={() => router.push('/functional-template-hub')}
          className={`flex items-center gap-2 rounded p-2 transition-colors duration-300 ${isDarkMode ? 'text-[#94A3B8] hover:bg-[#334155]' : 'text-[#64748B] hover:bg-[#F8FAFC]'}`}
        >
        ...
        <div className="flex items-center gap-3">
          <button
            aria-label="이전 레슨"
            className={`rounded p-1 transition-colors duration-300 ${isDarkMode ? 'text-[#94A3B8] hover:bg-[#334155]' : 'text-[#64748B] hover:bg-[#F8FAFC]'}`}
          >
            <ChevronLeft className="w-4 h-4" />
          </button>

          <span className={`text-sm font-medium transition-colors duration-300 ${isDarkMode ? 'text-[#94A3B8]' : 'text-[#64748B]'}`}>
            데이터 없음
          </span>

          <span className={`text-sm transition-colors duration-300 ${isDarkMode ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
            |
          </span>
